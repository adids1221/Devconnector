...
// @access Private
//TODO update the function so you can edit 1 matched experience
router.post('/experience/:exp_id',
    ...
    async (req, res) => {
        ...
        try {
            const profile = await Profile.findOne({ user: req.user.id });
            const upodateIndex = profile.experience.map(item => item.id).indexOf(req.params.exp_id);
            if (profile) {
                profile.experience.map(exp => {
                    if (exp._id == req.params.exp_id) {
                        //console.log(exp);
                        Object.keys(exp._doc).map(key => {
                            if (key == 'current') {
                                if (newExp[key] != exp[key])
                                    exp[key] = newExp[key];
                            }
                            else if (newExp[key]) {
                                exp[key] = newExp[key];
            ...
            if (profile) {
                profile.education.map(edu => {
                    if (edu._id == req.params.edu_id) {
                        Object.keys(edu._doc).map(key => {
                            if (key == 'current') {
                                if (newEdu[key] != edu[key])
                                    edu[key] = newEdu[key];
                            }
                            else if (newEdu[key]) {
                                edu[key] = newEdu[key];
